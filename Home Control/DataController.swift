...
class DataController: NSObject, LoggingObject {
    ...
    func evaluateDeciders(notification: NSNotification) {
        ...
        log("Checking deciders...")
        
        guard operationQueue.operationCount == 0 else {
            
            log("One or more devices are busy, postponing evaluation!")
        ...
        outerLoop: for device in devices {
            ...
            for decider in device.deciders {
                ...
                log("\(decider.name) wants \(device.name) to be \(decider.state).")
    ...
    private func reloadData(completion: RefreshCompletion?) {
        ...
        dispatch_async( dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0 ) ) {
            ...
            do {
                ...
                self.log("Parse fetch error: \(error)")
            }
            
            self.log("Reloaded all data sources.")
